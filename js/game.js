...
(function () {
  const P = TB.physics, C = TB.climate, V = TB.civ, S = TB.story,
        PR = TB.predict, U = TB.util;
  ...
  function resolveCalendar(state) {
    ...
    if (r === 'success') {
      ...
      if (cal.kind === 'stable') {
        ...
        const extremity = TB.util.clamp((Math.abs(T - 15) - 25) / 90, 0, 1);
        const frac = TB.util.clamp(0.12 + 0.4 * extremity, 0.05, 0.6);
